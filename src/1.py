...
class FakeChatOpenAI:
    """LangChain ChatOpenAI 的离线替身。

    初学者往往希望先理解初始化路径，而不是一开始就处理 API Key、网络代理、模型额度
    等问题。FakeChatOpenAI 保留 model / api_key / base_url / temperature 等关键字段，
    并提供 invoke 方法，从接口上模拟 LangChain ChatOpenAI。
    """

    def __init__(self, model: str, api_key: str, base_url: str = "", temperature: float = 0.0):
        # LangChain ChatOpenAI 常用字段：model/model_name 用于记录具体模型名称。
        self.model = model
        self.model_name = model
        self.api_key = api_key
        self.base_url = base_url
        self.temperature = temperature
        # 默认 provider 为 fake；build_chat_openai 会用配置里的 model_type 覆盖它。
        self.provider = "fake"

    def invoke(self, prompt: str) -> str:
        """模拟 LLM 连通性检查。

        真实 ChatOpenAI.invoke 会向模型服务发送请求。这里固定返回 OK，使离线测试可重复。
        """
        return "OK"
...
def build_chat_openai(config: AIModelConfig, use_real: bool = False) -> Any:
    ...
    if use_real:
        try:
            ...
            raise RuntimeError("langchain-openai is not installed. Install with: uv sync --extra real") from exc
...
class SafeRecordSaver:
    ...
    def __init__(self, save_func: Optional[Callable[[Dict[str, Any]], None]] = None):
...
class BrowserAgent:
    ...
    def __init__(
        ...
        # 学习版只实现文本智能模式，因此忽略传入 execution_mode，固定为 text。
        ...
        self.browser_profile = self.create_browser_profile()

        # action_plan 是学习版的“模型输出脚本”。为空时会自动把所有任务标记完成。
        self.action_plan = action_plan or []

        # pending_status_* 用来记录“上一步有业务动作但模型忘记标状态”的任务，下一步补偿。
        self.pending_status_task_id: Optional[Any] = None
        self.pending_status_task_description: Optional[str] = None

        # 认证失败计数：同一任务连续失败达到阈值后，立即标记失败并停止。
        self.auth_failure_task_id: Optional[Any] = None
    ...
    def verify_execution_llm(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Execution LLM unavailable: {exc}") from exc

    def create_browser_profile(self) -> BrowserProfileConfig:
        """创建 browser-use 浏览器配置。

        学习版固定按 Linux/WSL 环境生成配置；真实项目可根据部署环境传入不同 system 和
        chrome_path。
        """
        return create_browser_profile(system="Linux")
    ...
    def register_actions(self) -> Dict[str, Callable[..., Dict[str, Any]]]:
        ...
        def mark(task_id: int, status: str) -> Dict[str, Any]:
            """统一构造任务状态动作结果。"""
            return {"task_id": int(task_id), "status": status}

        return {
            # Done 表示整个浏览器任务结束。
            "Done": lambda success=True, text="": {"done": bool(success), "text": text},
            # close_tab 用于复刻“新标签页打开后关闭/切换”的动作能力。
            "close_tab": lambda: {"closed": True},
            # 显式任务状态动作：前端状态同步依赖这些动作。
    ...
    def build_execution_prompt(self, task_description: str, planned_tasks: List[Dict[str, Any]]) -> str:
        """构造强化后的执行提示词。
        ...
        """
        # 先清理 URL，避免中文标点粘在 URL 后导致浏览器访问错误地址。
        prompt = clean_task_urls(task_description)
        if planned_tasks:
            prompt += "\n\nIMPORTANT INSTRUCTION:\n"
            prompt += "You have a list of sub-tasks. Execute strictly in order.\n"
            prompt += (
                "CRITICAL: MUST call mark_task_complete, mark_task_failed, "
                "mark_task_skipped, or update_task_status immediately after each sub-task result.\n"
            )
            prompt += "Sub-tasks:\n"
            prompt += "\n".join(f"{task['id']}. {task['description']}" for task in planned_tasks)
        prompt += "\n\nCRITICAL PERFORMANCE & SYNC RULES:\n"
        prompt += "1. Mark the current task only after completion.\n"
        prompt += "2. If you mark task N, stop the current step; do not start N+1.\n"
        prompt += "3. Never invent credentials. Stop after repeated auth failures.\n"
        prompt += "4. If a link opens a new tab, switch to the newest tab.\n"
        prompt += "5. Use native action params: index, text, tab_id.\n"
        return prompt
    ...
    def run_task(
        ...
        """
        # 先做模型连通性预检查，避免浏览器流程启动后才失败。
        self.verify_execution_llm()

        planned_tasks = planned_tasks or []

        # 构造强化提示词。学习版不真正传给 browser-use，但保留调用以体现真实流程。
    ...
    def run_full_process(
        ...
        return self.run_task(task_description, planned_tasks, step_callback, should_stop)


def get_agent_class(execution_mode: str = "text"):
    """根据执行模式返回 Agent 类。
...
def run_ai_task_sync(
    ...
    **kwargs,
    ...
    return agent.run_task(task_description, planned_tasks, callback, should_stop)
